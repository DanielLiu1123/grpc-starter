...
主要提供了 gRPC server 相关的自动配置。

#### 入门指南

只需在 Spring Boot 项目中添加如下依赖即可：

```groovy
implementation 'com.freemanan:grpc-server-boot-starter:<latest>'
```

添加依赖后会默认启用 gRPC server，监听 `9090` 端口，可以通过 `grpc.server.port` 配置来修改端口号。
...
gRPC Starter
提供了一个简单的实现 [DefaultExceptionResolver](../../grpc-boot-autoconfigure/grpc-server-boot-autoconfigure/src/main/java/com/freemanan/starter/grpc/server/extension/exceptionhandling/DefaultExceptionHandler.java)
用作默认异常处理，可以通过 `grpc.server.exception-handling.use-default=true` 配置来开启，默认为关闭状态。

可以很容易地扩展自定义的异常处理实现，只需实现 `ExceptionHandler` 接口并且将其注册为 Spring Bean 即可：
...
这里可以实现一些异常上报的逻辑，比如上报到 [Sentry](https://sentry.io/welcome/)：
...
可以通过实现 `GrpcServerCustomizer` 接口来自定义 gRPC server 的配置，比如：
...
gRPC server 采用优雅关闭（等待请求处理完成后才关闭），可以通过 `grpc.server.shutdown-timeout` 配置优雅关闭的超时时间，默认为
5s；
0 表示无超时时间。
...
gRPC 提供了 in-process transport 来支持在同一个进程内使用 gRPC 进行通信。在测试 gRPC 服务时特别有用，
可以通过配置 `grpc.server.in-process.name` 来开启 in-process transport，默认为关闭状态。
...
2. 指定 gRPC stubs
   ...
   > `@EnableGrpcClients` 的用法与 Spring Cloud Openfeign `@EnableFeignClients` 非常相似。
...
集成 [protoc-gen-validate](https://github.com/bufbuild/protoc-gen-validate)
