...
:::tip
Since both WebFlux and gRPC utilize Netty, the grpc-starter now uses `grpc-netty` by default instead of `grpc-netty-shaded` to avoid increasing the size of the final JAR package by approximately 9MB.
