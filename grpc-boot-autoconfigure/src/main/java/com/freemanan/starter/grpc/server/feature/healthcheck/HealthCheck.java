...
@ConditionalOnProperty(prefix = GrpcProperties.Server.HealthCheck.PREFIX, name = "enabled", matchIfMissing = true)
public class HealthCheck {

    @Bean
    @ConditionalOnMissingBean
    public HealthImpl grpcHealthCheckImpl(ObjectProvider<HealthChecker> healthCheckers) {
        return new HealthImpl(healthCheckers);
    }

    @Configuration(proxyBeanMethods = false)
    @ConditionalOnProperty(
            prefix = GrpcProperties.Server.HealthCheck.DataSource.PREFIX,
            name = "enabled",
            matchIfMissing = true)
    ...
    static class DataSource {
        ...
        @Bean
        @ConditionalOnMissingBean
        public DataSourceHealthChecker grpcDataSourceHealthChecker(GrpcProperties properties) {
            return new DataSourceHealthChecker(
                    properties.getServer().getHealthCheck().getDatasource());
        }
    }

    @Configuration(proxyBeanMethods = false)
    @ConditionalOnProperty(
            prefix = GrpcProperties.Server.HealthCheck.Redis.PREFIX,
            name = "enabled",
            matchIfMissing = true)
